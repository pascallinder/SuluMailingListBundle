...
export default class UnsubscribeToolbarAction extends AbstractListToolbarAction {
    getToolbarItemConfig() {
        ...
        return {
            ...
            icon: 'fa-user-minus',
            label: translate('mailingListSubscription.action.unsubscribe'),
    ...
    @action handleClick = () => {
        ...
        store.selectionIds.forEach((id) => {
            updatePromises.push(
                ResourceRequester.post(this.listStore.resourceKey, null,{id: id,action: 'unsubscribe'})
                    .catch((error) => {
                        if (error.status !== 404) {
